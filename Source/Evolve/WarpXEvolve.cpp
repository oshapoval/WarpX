    ...
    for (int step = istep[0]; step < numsteps_max && cur_time < stop_time; ++step)
    {
             ...
             electromagnetic_solver_id == ElectromagneticSolverAlgo::HybridPIC )
        {
            ...
            if (collisions_placement == 1){
                ExecutePythonCallback("beforecollisions");
                mypc->doCollisions( cur_time, cur_time, dt[0] );
                ExecutePythonCallback("aftercollisions");
            }
...
WarpX::OneStep_nosub2 (Real cur_time)
{
    ...
    if (WarpX::electromagnetic_solver_id == ElectromagneticSolverAlgo::PSATD) {
        ...
        if (do_synchronized){
            ...
            EvolveB(dt[0], DtType::Full, cur_time); // We now have B^{n+1}
        }
        ...
        if (do_synchronized){
            EvolveB(0.5_rt * dt[0], DtType::SecondHalf, cur_time + 0.5_rt * dt[0]); // We now have B^{n+1}// olga : need to check cur_time + 0.5_rt * dt[0]?
        }
