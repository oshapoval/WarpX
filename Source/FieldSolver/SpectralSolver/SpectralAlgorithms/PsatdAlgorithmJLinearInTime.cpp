        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            if (time_averaging)
            {
                ...
                if (divb_cleaning)
                {
                    ...
                    fields(i,j,k,Idx.By_avg) += I * ep0 * X1 * c2 * G_old * ky;
                    fields(i,j,k,Idx.Bz_avg) += I * ep0 * X1 * c2 * G_old * kz;
                }
            }
        });
    }
