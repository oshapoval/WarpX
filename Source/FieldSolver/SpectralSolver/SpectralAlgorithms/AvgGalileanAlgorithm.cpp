        ...
        [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
        {
            ...
            if (k_norm != 0){
                ...
                if ( (nu != 1.) && (nu != 0.) ) {
                    ...
                    // std::complex<long double> Psi02[17][31][0], iu(0.l,1.l);
                    // std::complex<long double> Psi_02;
                    // long double nu_ld = (long double)nu, kv_ld =(long double)kv, dt_ld =(long double)dt  ;
                    // long double c_ld = (long double)c, k_norm_ld = (long double)k_norm;
                    // long double wdt_ld = c_ld*dt_ld*k_norm_ld;
                    // Psi_02 = std::exp( 0.5l*iu*kv_ld*dt_ld ) * ( (std::cos(wdt_ld/2.l) - iu*nu_ld*std::sin(wdt_ld/2.l)) - std::exp( iu*kv_ld*dt_ld )* (std::cos(1.5l*wdt_ld)- iu*nu_ld*std::sin(1.5l*wdt_ld) )) / (wdt_ld * (nu_ld*nu_ld - 1.l)) ;
                    // Psi2(i,j,k) = Psi02[i][j][k];
                    // Psi2(i,j,k) = Psi_02;
                ...
                if ( nu == 0) {
                    ...
                    // Coefficients ofr averaged algo
                    EpJmult(i,j,k) = Psi2(i,j,k)/(ep0*c*k_norm);;
                    BpJmult(i,j,k) = -k_norm*I*A1(i,j,k)/ep0;
                    Rhocoef(i,j,k) = -I*c2 / (ep0*dt*vgal_norm);
...
// #endif
            ...
            Print()<<"Psi1"<< ' ' << Psi2 << i <<' '<< j <<' '<< kx << ' '<< kz << ' '<<'\n';
            //Print()<<"kz"<< ' ' << i <<' '<< j <<' '<<kz<<'\n';

            //Print()<<"By:"<< ' '<< i << ' '<<j << ' '<< fields(i,j,k,Idx::By)<<' ||' << fields(i,j,k,Idx::By_avg)<<' '<<'\n';
